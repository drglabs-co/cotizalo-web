...
add_filter('pre_handle_404', function ($preempt, $wp_query) {
    ...
    if (isset($routes[$uri]) || $uri === 'favicon.ico') {
...
add_action('template_redirect', function () {
    ...
    // Special handler for root favicon.ico requests
    if ($uri === 'favicon.ico') {
        $favicon = get_template_directory() . '/favicon.ico';
...
/**
 * Hook into WordPress internal robots handler as fallback.
 */
add_action('do_robotstxt', function () {
    $robots_template = get_template_directory() . '/robots-txt.php';
    if (file_exists($robots_template)) {
        include $robots_template;
        exit;
    }
});
